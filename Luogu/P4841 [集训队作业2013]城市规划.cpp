...
int main() {
    ...
    int n;
    std::cin >> n;
    std::vector<Pot> fac(n + 1), invfac(n + 1);
    fac[0] = invfac[0] = 1;
    for(int i = 1; i <= n; ++i) {
        fac[i] = fac[i - 1] * i;
    }
    ...
    std::vector<Pot> a(n + 1), b(n + 1);
    for(int i = 1; i <= n; ++i) {
        b[i] = power(Pot(2), (long long) i * (i - 1) / 2 % (mod - 1)) * invfac[i - 1];
    }
    for(int i = 0; i <= n; ++i) {
        a[i] = power(Pot(2), (long long) i * (i - 1) / 2 % (mod - 1)) * invfac[i];
    }
    auto res = Poly(a).inv(n + 1) * Poly(b);
    std::cout << res[n] * fac[n - 1] << std::endl; 
    return 0;
}